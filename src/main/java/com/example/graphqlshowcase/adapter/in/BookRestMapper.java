...
public class BookRestMapper {
	...
	private static PublisherResponseDto mapPublisherToPublisherResponseDto(final Publisher publisher) {
	...
	public static Book mapUpdateBookRequestDtoToBook(final UpdateBookRequestDto bookRequestDto) {
		...
		return new Book(bookRequestDto.getId(), bookRequestDto.getTitle(), isbn, authors, genre,
				mapPublisherRequestDtoToPublisher(bookRequestDto.getPublisher()));
	}
	
	public static Publisher mapPublisherRequestDtoToPublisher(final PublisherRequestDto publisherRequestDto) {
		return new Publisher(publisherRequestDto.getName(), 
	...
	private static Address mapAddressRequestDtoToAddress(final AddressRequestDto addressRequestDto) {
		return new Address(addressRequestDto.getHouseNumber(), 
	...
	public static List<Author> mapAuthorRequestDtosToAuthors(final List<AuthorRequestDto> authorRequestDtos) {
		return authorRequestDtos.stream().map(BookRestMapper::mapAuthorRequestDtoToAuthor).collect(Collectors.toList());
	}
	
	private static Author mapAuthorRequestDtoToAuthor(final AuthorRequestDto authorRequestDto) {
		return new Author(authorRequestDto.getFirstName(),
				authorRequestDto.getLastName(), authorRequestDto.getEmail());
	}

	private static List<AuthorResponseDto> mapAuthorsToAuthorResponseDtos(final List<Author> authors) {
		return authors.stream().map(BookRestMapper::mapAuthorToAuthorResponseDto).collect(Collectors.toList());
