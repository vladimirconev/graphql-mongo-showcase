...
public class BookApplicationStartupListener implements ApplicationListener<ContextRefreshedEvent> {
	...
	public void onApplicationEvent(ContextRefreshedEvent event) {
		if(initialDataLoad) {
			...
			var pearsonHeadquatersAddress = new Address("23A", "United Kingdom", "London", "EC1A", "88 Park Road");
			var relxHeadquatersAddress = new Address("76/2A", "United Kingdom", "London", "EC5A", "34 London St Road");
			var thomsonReutersHeadquatersAddress = new Address("2A", "Canada", "Toronto", "66777", "Main Road str");
			...
			var isbnList = List.of(new ISBN("978-1-891830-25-9"), new ISBN("978-1-60309-047-6"),
					new ISBN("978-1-891830-85-3"), new ISBN("978-1-60309-478-8"), new ISBN("978-1-891830-85-3"),
					new ISBN("978-1-60309-369-9"));
			
			Random random = new Random();
			
			bookRepository.createBook(isbnList.get(random.nextInt(isbnList.size())), Genre.FANTASY, 
					"Cloud Hotel", List.of(foo,boo), pearson);
			bookRepository.createBook(isbnList.get(random.nextInt(isbnList.size())), Genre.CONTEMPORARY, 
					"Dragon Puncher Book1", List.of(elaineBloyd), relx);
			bookRepository.createBook(isbnList.get(random.nextInt(isbnList.size())), Genre.SCI_FI, 
					"Johny Boo", List.of(mitchTimotchy), penguin);
			bookRepository.createBook(isbnList.get(random.nextInt(isbnList.size())), Genre.MYSTERY, 
					"River of Ghosts", List.of(antonioVilla), hacheteLivre);
			bookRepository.createBook(isbnList.get(random.nextInt(isbnList.size())), Genre.ROMANCE, 
					"Our Expanding Universe", List.of(foo), thomsonReuters);
			
			bookRepository.createBook(isbnList.get(random.nextInt(isbnList.size())), Genre.TRILLER, 
					"Ninja Chicken", List.of(foo, boo, mitchTimotchy), penguin);
			
			bookRepository.createBook(isbnList.get(random.nextInt(isbnList.size())), Genre.DYSTOPIAN, 
					"A clockwork Orange", List.of(foo, boo), pearson);
